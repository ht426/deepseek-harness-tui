/**
 * `@`-file-mention detection and candidate filtering for the input bar — the
 * `@` counterpart to `input-bar.tsx`'s `/`-command query, applied around the
 * cursor instead of anchored to the start of the line so a mention can open
 * mid-sentence.
 */
 ...
 * @param value - the full input buffer.

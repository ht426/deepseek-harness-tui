/**
 * Repo-relative file paths for the `@`-mention dropdown (`input-bar.tsx`).
 * Reader-local convenience, so this goes straight to `node:child_process`/
 * `node:fs` rather than through any agent-facing sandboxed fs seam — a
 * different concern from listing files for local autocomplete.
 */
